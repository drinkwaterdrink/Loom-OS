...
export const CustomModuleSchema = z.object({
  ...
  javascriptTemplate: z.string().max(80000).optional(),
...
export const StockModuleOverrideSchema = z.object({
  ...
  htmlTemplate: z.string().max(12000).optional(),
  cssTemplate: z.string().max(12000).optional(),
}).strict();
...
const RawSettingsSchema = z.object({
  ...
  historyRetentionLimit: z.number().int().min(1).max(1000).default(100),
  generationTimeoutSeconds: z.number().int().min(30).max(300).default(180),
  connectionId: z.string().trim().max(200).default(""),
  activeThemeId: z.string().trim().max(160).default(""),
  developerMode: z.boolean().default(false),
  viewerTemplateEnabled: z.boolean().default(false),
  viewerHtmlTemplate: z.string().max(16000).default(""),
  viewerCssTemplate: z.string().max(16000).default(""),
...
const ClothingSchema = z.object({
  ...
  layerCount: z.number().int().min(0).max(8).optional().default(0),
  layers: z.array(LayerSchema).max(8).optional().default([]),
}).strict();
...
export const ToolsSchema = z.object({
  ...
  imagePrompt: z.object({
    ...
    characterContinuity: z.string().trim().max(4000).optional().default(""),
    ...
    constraints: z.array(MediumText).max(16).optional().default([]),
    full: z.string().trim().max(8000),
    hint: MediumText,
  }).strict().nullable(),
}).strict();
